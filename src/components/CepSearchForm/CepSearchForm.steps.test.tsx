...
describeFeature(feature, ({ Background, Scenario, AfterEachScenario }) => {
  ...
  Scenario('Busca de um CEP válido e existente', ({ When, And, Then }) => {
    When(
      'o usuário digita {string} no campo de CEP',
      async (_ctx: TestContext, cep: string) => {
    ...
    Then(
      'o endereço {string} deve ser exibido',
      async (_ctx: TestContext, logradouro: string) => {
        expect(await screen.findByText(logradouro)).toBeInTheDocument();
      },
    );

    And(
      'a cidade {string} deve ser exibida',
      (_ctx: TestContext, cidade: string) => {
  ...
  Scenario('Busca de um CEP inexistente', ({ When, And, Then }) => {
    ...
    Then(
      'uma mensagem informando que o CEP não foi encontrado deve ser exibida',
      async () => {
  ...
  Scenario(
    'Falha de conexão ao consultar o serviço de CEP',
    ({ Given, When, And, Then }) => {
